...
z0_scaled = solve_loads_newton(model, x0_scaled, u0_scaled, kappa0, z0_scaled, true);
...
fprintf('Running fixed-step RK4 simulation...\n');
...
z_new = solve_loads_newton(model, xs, u_scaled, kappa, z_guess, false);
...
%% ------------------------------------------------------------------------
function z = solve_loads_newton(model, xs, us, kappa, z_guess, do_warn)
z = z_guess(:);
z = min(max(z, model.z_min), model.z_max);

r = full(model.f_hload(xs, us, z, kappa));
r = r(:);
if norm(r, inf) < model.veh.eps1
    return;
end
...
for iter = 1:12
    ...
    for j = 1:numel(z)
        dz = 1e-6 * max(1.0, abs(z(j)));
        zp = z;
        zp(j) = min(max(zp(j) + dz, model.z_min(j)), model.z_max(j));
        dz_actual = zp(j) - z(j);
        if abs(dz_actual) < eps
            zp(j) = z(j) - dz;
            zp(j) = min(max(zp(j), model.z_min(j)), model.z_max(j));
            dz_actual = zp(j) - z(j);
        end
        rp = full(model.f_hload(xs, us, zp, kappa));
        J(:,j) = (rp(:) - r) / dz_actual;
    end
    ...
    for bt = 1:8
        ...
        if norm(r_trial(:), inf) <= 0.9*rnorm || norm(r_trial(:), inf) < 1e-8
            z = z_trial;
            accepted = true;
            break;
        end
    ...
    if ~accepted
        z = min(max(z + 0.25*step, model.z_min), model.z_max);
    end
end

r = full(model.f_hload(xs, us, z, kappa));
if do_warn || norm(r(:), inf) > 1e-5
    warning('Tyre-load Newton residual max |h_load| = %.3e.', norm(r(:), inf));
end
end
...
for k = 1:N
    ...
    zs = solve_loads_newton(model, xs, us, kap, z_guess, false);
    z_guess = zs;
