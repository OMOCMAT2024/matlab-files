...
% J = dt * ones(N,1);
% J = dt * ones(N,1) + sumsqr(repmat(rdy,1,N).*dzk);
% J = dt * ones(N,1) + sumsqr(repmat(rdu,1,N).*duk);
...
% du_jump = Uk(:,2:N+1) - Uk(:,1:N);
dz_jump = Zk(:,2:N+1) - Zk(:,1:N);

J = dt * ones(N,1) ...
  + eps_u * sumsqr(repmat(rdu,1,N).*du_jump) ...
  + eps_z * sumsqr(repmat(rdy,1,N).*dz_jump);
