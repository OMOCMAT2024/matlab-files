...
k_branchblend            = 5.0;   % larger = sharper shifts
num_interpolation_points = 100000;

% Optional output controls
save_symbolic_text_files = true;  % derivative expressions can be very large
zoom_half_width          = 1.0;   % rad/s, for zoomed plots around each shift

% -------------------------------------------------------------------------
% ADDED: USER-SUPPLIED ARRAY OF KNOWN REAR AXLE SPEEDS [rad/s]
% Replace this with your own array whenever needed
% -------------------------------------------------------------------------
% omega_rear_axle_query = [ ...
%      5
%     15
%     25
%     35
...
load('D:\Applications\MYOC_test\MYOC-main\tro\tro_sol\sol_Catalunya.mat')
omega_rear_axle_query = sol.u_opt(4,:).';
...
validateattributes(omega_rear_axle_query, {'double'}, {'real','finite','vector','nonnegative','nonempty'});
...
%% =========================================================================
% ADDED: EXACT PIECEWISE GEAR / ENGINE-SPEED LOOKUP FOR USER ARRAY
% =========================================================================
[gear_query, omega_engine_query_rads, omega_engine_query_rpm, ratio_query] = ...
    evaluate_exact_piecewise_gear_engine_map( ...
        omega_rear_axle_query, model.total_ratios, model.shifts_rads);

P_query_piecewise = polyval(model.engine_poly_coeffs, omega_engine_query_rads);
...
%% =========================================================================
% ADDED: PRINT RESULT TABLE FOR USER ARRAY
% =========================================================================
fprintf('\n============================================================\n');
fprintf('EXACT PIECEWISE GEAR / ENGINE-SPEED LOOKUP FOR USER ARRAY\n');
fprintf('============================================================\n');
...
lookup_table = table( ...
    ...
    gear_query(:), ...
    ratio_query(:), ...
    omega_engine_query_rads(:), ...
    omega_engine_query_rpm(:), ...
    P_query_piecewise(:), ...
    'VariableNames', { ...
        'omega_rear_axle_rad_s', ...
        'gear', ...
        'active_total_ratio', ...
        'omega_engine_rad_s', ...
        'omega_engine_rpm', ...
...
title('Automatically generated power_E_max(\omega_a)');
legend('Smooth automatically generated power_E_max', ...
       'Exact piecewise 8-speed reference', ...
       'Location', 'southeast');
...
xlabel('Rear axle speed \omega_a (rad/s)');
ylabel('Power (kW)');
title('Smooth power_E_max(\omega_a)');
legend('Smooth power_E_max', 'Exact piecewise 8-speed', 'Location', 'best');
...
title('First derivative of smooth power_E_max');
...
title('Second derivative of smooth power_E_max');
legend('Symbolic exact derivative', 'Finite-difference check from P', 'Location', 'best');
...
for i = 1:nShifts
    ...
    legend('Smooth power_E_max', 'Exact piecewise', 'Location', 'best');
end
...
%% =========================================================================
% ADDED: PLOTS FOR USER-SUPPLIED REAR-AXLE-SPEED ARRAY
% =========================================================================
omega_rear_axle_query_col   = omega_rear_axle_query(:);
omega_engine_query_rads_col = omega_engine_query_rads(:);
omega_engine_query_rpm_col  = omega_engine_query_rpm(:);
gear_query_col              = gear_query(:);

[omega_rear_axle_query_sorted, sort_idx_query] = sort(omega_rear_axle_query_col, 'ascend');
omega_engine_query_rads_sorted = omega_engine_query_rads_col(sort_idx_query);
omega_engine_query_rpm_sorted  = omega_engine_query_rpm_col(sort_idx_query);
gear_query_sorted              = gear_query_col(sort_idx_query);

figure('Color', 'w', 'Position', [120, 60, 1400, 950]);
t_lookup = tiledlayout(3,1, 'TileSpacing', 'Compact', 'Padding', 'Compact');

% Engine speed in rad/s
nexttile; hold on; grid on; box on;
plot(omega_rear_axle_query_sorted, omega_engine_query_rads_sorted, 'bo-', ...
    'LineWidth', 2.0, 'MarkerSize', 6, 'MarkerFaceColor', 'b');
for i = 1:numel(model.shifts_rads)
    xline(model.shifts_rads(i), 'k:');
end
xlabel('Rear axle speed \omega_a (rad/s)');
ylabel('Engine speed \omega_e (rad/s)');
title('Exact piecewise engine speed from user rear-axle-speed array');
legend('Engine speed (rad/s)', 'Location', 'best');

% Engine speed in rpm
nexttile; hold on; grid on; box on;
plot(omega_rear_axle_query_sorted, omega_engine_query_rpm_sorted, 'rs-', ...
    'LineWidth', 2.0, 'MarkerSize', 6, 'MarkerFaceColor', 'r');
for i = 1:numel(model.shifts_rads)
    xline(model.shifts_rads(i), 'k:');
end
yline(engine_max_rpm, 'k--', 'Engine max rpm', ...
    'LabelHorizontalAlignment', 'left', ...
    'LabelVerticalAlignment', 'bottom');
xlabel('Rear axle speed \omega_a (rad/s)');
ylabel('Engine speed (rpm)');
title('Exact piecewise engine speed in rpm from user rear-axle-speed array');
legend('Engine speed (rpm)', 'Location', 'best');

% Gear position
nexttile; hold on; grid on; box on;
stairs(omega_rear_axle_query_sorted, gear_query_sorted, 'm-', 'LineWidth', 2.2);
plot(omega_rear_axle_query_sorted, gear_query_sorted, 'ko', ...
    'MarkerFaceColor', 'm', 'MarkerSize', 5);
for i = 1:numel(model.shifts_rads)
    xline(model.shifts_rads(i), 'k:');
end
ylim([0.5, numel(model.total_ratios)+0.5]);
yticks(1:numel(model.total_ratios));
xlabel('Rear axle speed \omega_a (rad/s)');
ylabel('Gear position');
title('Exact piecewise selected gear from user rear-axle-speed array');
legend('Gear position', 'Sample points', 'Location', 'best');

title(t_lookup, 'Exact Piecewise Gear / Engine-Speed Lookup for User Array', 'FontSize', 16);
...
function [gear_idx, omega_engine_rads, omega_engine_rpm, active_ratio] = ...
    evaluate_exact_piecewise_gear_engine_map(omega_a_in, total_ratios, shifts_rads)

    input_size = size(omega_a_in);
    omega_a_col = omega_a_in(:);
    ratio_col   = total_ratios(:);
    nGears = numel(ratio_col);

    validateattributes(omega_a_col, {'double'}, {'real','finite','nonnegative'});
    validateattributes(ratio_col, {'double'}, {'real','finite','vector','positive'});
    validateattributes(shifts_rads, {'double'}, {'real','finite','vector','positive'});

    assert(numel(shifts_rads) == nGears - 1, ...
        'Number of shift points must be nGears - 1.');
    assert(all(diff(ratio_col) < 0), ...
        'total_ratios must be strictly decreasing.');
    assert(all(diff(shifts_rads) > 0), ...
        'shifts_rads must be strictly increasing.');

    gear_idx = zeros(size(omega_a_col));
    bounds = [0; shifts_rads(:); inf];

    % Exact piecewise rule:
    % gear 1 for 0 <= omega_a < shift_1
    % gear 2 for shift_1 <= omega_a < shift_2
    % ...
    % gear n for shift_(n-1) <= omega_a < inf
    for i = 1:nGears
        idx = (omega_a_col >= bounds(i)) & (omega_a_col < bounds(i+1));
        gear_idx(idx) = i;
    end

    % Safety fallback for numerical edge cases
    gear_idx(gear_idx < 1) = 1;
    gear_idx(gear_idx > nGears) = nGears;

    active_ratio_col = ratio_col(gear_idx);
    omega_engine_rads_col = active_ratio_col .* omega_a_col;
    omega_engine_rpm_col  = omega_engine_rads_col * 30 / pi;

    gear_idx          = reshape(gear_idx,             input_size);
    active_ratio      = reshape(active_ratio_col,     input_size);
    omega_engine_rads = reshape(omega_engine_rads_col,input_size);
    omega_engine_rpm  = reshape(omega_engine_rpm_col, input_size);
end
