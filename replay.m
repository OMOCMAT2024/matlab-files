...
hcg = 0.45;                  % nominal COM height above flat ground
...
insetAxPos = [0.06 0.08 0.26 0.26];   % bottom-left 2D inset

% ----- HUD layout / animation -----
hudAxPos = [0.34 0.72 0.32 0.23];     % upper-middle overlay
steeringWheelRatio = 14;              % steering wheel angle = ratio * deltaFL
...
speedAxPos       = [0.73 0.73 0.22 0.20];   % top-right overlay
...
requiredVars = {'leftX','leftY','rightX','rightY', ...
                't','carX','carY','speed','heading','roll','pitch', ...
                'deltaFL','deltaFR', ...
                'omegaFL','omegaFR','omegaRL','omegaRR', ...
                'Tdrive_norm','Tbrake_norm'};
...
sameLengthVars = {'carX','carY','carZ','speed','heading','roll','pitch', ...
                  'deltaFL','deltaFR','omegaFL','omegaFR','omegaRL','omegaRR', ...
                  'Tdrive_norm','Tbrake_norm'};
...
allFinite = all(isfinite(t))      && all(isfinite(carX))    && all(isfinite(carY)) && ...
            all(isfinite(carZ))   && all(isfinite(speed))   && all(isfinite(heading)) && ...
            all(isfinite(roll))   && all(isfinite(pitch))   && ...
            all(isfinite(deltaFL))&& all(isfinite(deltaFR)) && ...
            all(isfinite(omegaFL))&& all(isfinite(omegaFR)) && ...
            all(isfinite(omegaRL))&& all(isfinite(omegaRR)) && ...
            all(isfinite(Tdrive_norm)) && all(isfinite(Tbrake_norm)) && ...
            all(isfinite(leftX))  && all(isfinite(leftY))   && all(isfinite(leftZ)) && ...
            all(isfinite(rightX)) && all(isfinite(rightY))  && all(isfinite(rightZ));
...
if canDrawRoadSurface
    ...
    surf(ax3, Xroad, Yroad, Zroad, ...
        'FaceColor', roadColor, 'EdgeColor', 'none', 'FaceAlpha', 1.0);
end
...
% Body patch
patch('Parent', hBodyTF, ...
      'Vertices', vertsBodyLocal, ...
      'Faces', facesBodyLocal, ...
      'FaceColor', carFaceColor, ...
      'EdgeColor', carEdgeColor, ...
      'LineWidth', 1.0, ...
      'FaceAlpha', carBodyAlpha);
...
for iw = 1:4
    ...
    surf('Parent', hWheelSpin(iw), ...
        'XData', XwLoc, 'YData', YwLoc, 'ZData', ZwLoc, ...
        'FaceColor', wheelColor3D, 'EdgeColor', wheelEdgeColor3D, ...
        'FaceAlpha', 1.0);

    line('Parent', hWheelSpin(iw), ...
        'XData', wheelMarkerLine(:,1), ...
        'YData', wheelMarkerLine(:,2), ...
        'ZData', wheelMarkerLine(:,3), ...
        'Color', wheelMarkerColor, 'LineWidth', 2.2);
end
...
xlim(ax2, xL);
ylim(ax2, yL);
set(ax2, 'Color', 'w', 'LineWidth', 1.0, 'Layer', 'top');
set(ax2, 'XTick', [], 'YTick', []);
title(ax2, 'Top view', 'FontSize', 10);

plot(ax2, leftX,  leftY,  'k-', 'LineWidth', 0.9);
plot(ax2, rightX, rightY, 'k-', 'LineWidth', 0.9);
plot(ax2, xv, yv, '-', 'Color', [0.80 0.80 0.80], 'LineWidth', 0.7);

hTrail2 = plot(ax2, NaN, NaN, '-', 'Color', trailColor, 'LineWidth', 1.2);

hCar2D = patch(ax2, 'XData', NaN, 'YData', NaN, ...
    'FaceColor', carFaceColor, 'EdgeColor', 'k', 'LineWidth', 0.9, ...
    'FaceAlpha', carBodyAlpha);
...
for iw = 1:4
    hWheel2D(iw) = patch(ax2, 'XData', NaN, 'YData', NaN, ...
        'FaceColor', wheelColor2D, 'EdgeColor', 'k', 'LineWidth', 0.7);
end
...
patch(axHUD, [0.02 0.98 0.98 0.02], [0.04 0.04 0.96 0.96], hudBgColor, ...
    'FaceAlpha', 0.70, 'EdgeColor', hudFrameColor, 'LineWidth', 1.2);
...
% Rim and hub
plot(axHUD, swCenter(1) + swRimR*cos(tt), ...
            swCenter(2) + swRimR*sin(tt), ...
            '-', 'Color', hudFrameColor, 'LineWidth', 4.0);

patch(axHUD, ...
    swCenter(1) + swHubR*cos(tt), ...
    swCenter(2) + swHubR*sin(tt), ...
    hudFrameColor, 'EdgeColor', hudFrameColor);

% Three spokes
hSWSpokes = gobjects(3,1);
for ispk = 1:3
    hSWSpokes(ispk) = line(axHUD, NaN, NaN, ...
        'Color', hudFrameColor, 'LineWidth', 3.0);
end

% Rotating rim marker so the steering wheel animation is more obvious
swMarkerR1 = swRimR - 0.015;
swMarkerR2 = swRimR + 0.010;
hSWMarker = line(axHUD, NaN, NaN, ...
    'Color', wheelMarkerColor, 'LineWidth', 3.0);
...
% Bar backgrounds
patch(axHUD, driveBarX, barYFrame, [1 1 1], ...
    'FaceAlpha', 0.75, 'EdgeColor', hudFrameColor, 'LineWidth', 1.4);

patch(axHUD, brakeBarX, barYFrame, [1 1 1], ...
    'FaceAlpha', 0.75, 'EdgeColor', hudFrameColor, 'LineWidth', 1.4);

% Dynamic bar fills
hDriveBarFill = patch(axHUD, driveBarX, [barY0 barY0 barY0 barY0], driveBarColor, ...
    'EdgeColor', 'none', 'FaceAlpha', 0.95);

hBrakeBarFill = patch(axHUD, brakeBarX, [barY0 barY0 barY0 barY0], brakeBarColor, ...
    'EdgeColor', 'none', 'FaceAlpha', 0.95);

% HUD labels
text(axHUD, driveBarXC, 0.10, 'Drive', ...
    'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'FontWeight', 'bold', 'Color', hudTextColor, 'FontSize', 10);

text(axHUD, brakeBarXC, 0.10, 'Brake', ...
    'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'FontWeight', 'bold', 'Color', hudTextColor, 'FontSize', 10);

text(axHUD, swCenter(1), 0.92, 'Steering', ...
    'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'FontWeight', 'bold', 'Color', hudTextColor, 'FontSize', 11);

hSWText = text(axHUD, swCenter(1), 0.16, '', ...
    'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'FontWeight', 'bold', 'Color', hudTextColor, 'FontSize', 10);

% ------------------------- Speed mini-plot axes -------------------------
axSpd = axes('Parent', fig, 'Position', speedAxPos);
hold(axSpd,'on'); grid(axSpd,'on'); box(axSpd,'on');
xlim(axSpd, [tv(1) tv(end)]);
ylim(axSpd, speedYL);
set(axSpd, 'Color', speedAxBgColor, 'LineWidth', 1.0, 'Layer', 'top');
...
hSpeedCursor = line(axSpd, [NaN NaN], speedYL, ...
    'Color', [0.55 0.55 0.55], 'LineStyle', '--', 'LineWidth', 1.0);
hSpeedMarker = plot(axSpd, NaN, NaN, 'o', ...
    'MarkerSize', 6, 'MarkerFaceColor', speedMarkerColor, ...
    'MarkerEdgeColor', [0.20 0.20 0.20], 'LineWidth', 1.0);

hSpeedVal = text(axSpd, NaN, NaN, '', ...
    'FontWeight', 'bold', 'Color', speedTextColor, ...
    'BackgroundColor', [1 1 1], 'Margin', 2, ...
    'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom', ...
    'Clipping', 'on');

% ------------------------- Figure annotation -------------------------
ann = annotation(fig, 'textbox', [0.02 0.84 0.27 0.14], ...
...
for k = 1:Nv
    ...
    set(hCar2D, 'XData', carXY_world(:,1), 'YData', carXY_world(:,2));

    if showHeadingArrow2D
        noseXY_world = (R2 * noseXY_local_2D.').';
        noseXY_world(:,1) = noseXY_world(:,1) + xc;
        noseXY_world(:,2) = noseXY_world(:,2) + yc;
        set(hNose2D, 'XData', noseXY_world(:,1), 'YData', noseXY_world(:,2));
    end
    ...
    for iw = 1:4
        ...
        set(hWheel2D(iw), 'XData', ptsWorld(:,1), 'YData', ptsWorld(:,2));
    end

    % 2D trail
    set(hTrail2, 'XData', xv(k0:k), 'YData', yv(k0:k));
    ...
    set(hSWText, 'String', sprintf('\\delta_{FL} = %.1f^\\circ', rad2deg(deltaFLv(k))));
    ...
    set(hSpeedVal, 'Position', [xText yText 0], ...
                   'String', sprintf('%.2f m/s', vv(k)));
