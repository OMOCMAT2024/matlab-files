...
nexttile
plot(track.s,sol.x_opt(8,:),'-'), grid on, hold on
plot(track.s,sol.x_opt(9,:),'--')
plot(track.s,sol.x_opt(10,:),'-.')
...
hold on
plot(track.s,sol.acc(:,1))
hold on
plot(track.s,sol.acc(:,2))
grid on
ylabel('[m/s2]')
legend('ax_{bar}','ay_{bar}', 'ax', 'ay')
xlim([0 s_end])
xlabel('s [m]')
...
nexttile
plot(track.s,sol.aero(:,1)), grid on, hold on
plot(track.s,sol.aero(:,2))
ylabel('[N]')
xlim([0 s_end])
...
scatter(sol.track_opt(:,1),sol.track_opt(:,2),5,rad2deg(sol.x_opt(2,:))), hold on
