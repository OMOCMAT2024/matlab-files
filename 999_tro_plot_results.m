...
f1 = figure;
fig_states = tiledlayout(4,2,'TileSpacing','Compact','Padding','Compact');

nexttile
plot(track.s,sol.x_opt(1,:)), grid on
hold on
plot(track.s,sol.x_opt(2,:))
ylabel('[m/s]')
legend('u','v')
xlim([0 s_end])

nexttile
Vmag = sqrt(sol.x_opt(1,:).^2 + sol.x_opt(2,:).^2);
plot(track.s,Vmag*3.6), grid on
ylabel('speed [km/h]')
xlim([0 s_end])
...
nexttile
plot(track.s,sol.x_opt(8,:)), grid on
ylabel('n [m]')
xlim([0 s_end])

nexttile
plot(track.s,sol.x_opt(9,:)), grid on
ylabel('\xi [rad]')
xlim([0 s_end])

nexttile
plot(track.s,sol.x_opt(4,:)), grid on, hold on
plot(track.s,sol.x_opt(6,:))
ylabel('[rad]')
legend('\phi roll','\theta pitch')
xlim([0 s_end])

nexttile
plot(track.s,sol.x_opt(5,:)), grid on, hold on
...
nexttile
plot(track.s,sol.x_opt(10,:),'-'), grid on, hold on
plot(track.s,sol.x_opt(11,:),'--')
plot(track.s,sol.x_opt(12,:),'-.')
plot(track.s,sol.x_opt(13,:),':')
ylabel('[rad/s]')
legend('\omega_{fl}','\omega_{fr}','\omega_{rl}','\omega_{rr}')
xlim([0 s_end])
xlabel('s [m]')

sgtitle('Chapter 9 State Variables')

%% Control and Auxiliary Variables
f2 = figure;
fig_control = tiledlayout(3,2,'TileSpacing','Compact','Padding','Compact');
...
nexttile
plot(track.s,sol.z_opt(1,:),'-'), grid on, hold on
plot(track.s,sol.z_opt(2,:),'--')
ylabel('front F_z [N]')
legend('Fz_{fl}','Fz_{fr}')
xlim([0 s_end])

nexttile
plot(track.s,sol.z_opt(3,:),'-'), grid on, hold on
plot(track.s,sol.z_opt(4,:),'--')
ylabel('rear F_z [N]')
legend('Fz_{rl}','Fz_{rr}')
xlim([0 s_end])

nexttile
if isfield(sol,'load')
    plot(track.s,sol.load(:,1:4)), grid on
    ylabel('load residual / Fz_s')
    legend('fl','fr','rl','rr')
end
xlim([0 s_end])
xlabel('s [m]')
...
nexttile
if isfield(sol,'acc') && size(sol.acc,2) >= 7
    plot(track.s,sol.acc(:,6)), grid on, hold on
    plot(track.s,sol.acc(:,7))
    ylabel('[rad/s^2]')
    legend('$\ddot{\phi}$','$\ddot{\theta}$','Interpreter','latex')
end
xlim([0 s_end])
xlabel('s [m]')

sgtitle('Controls and Chapter 9 Algebraic Loads')
...
scatter(sol.track_opt(:,1),sol.track_opt(:,2),5,Vmag*3.6), hold on
...
saveas(f7,'figures/workloads.png')
