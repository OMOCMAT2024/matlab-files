...
% Dynamic load and front/rear elastic roll contributions.
% Anti-roll bars are represented as additional axle roll stiffnesses:
%   Kphi_f = Kphi_F_spr + Kphi_F_arb,
%   Kphi_r = Kphi_R_spr + Kphi_R_arb.
% The total roll DOF uses Kphi = Kphi_f + Kphi_r, so the ARB is not
% double-counted. If the new fields are absent, the code falls back to the
% original ratio-based split.
Mtheta_sus2unspr_dyn = veh.Ctheta * theta_pitch_dot + veh.Ktheta * theta_pitch;

Kphi_f = getvehfield_any(veh, {'Kphi_F_total','Kphi_f','Kphi_front'}, ...
          veh.Kphi * veh.dis_Kphi_ratio);
Kphi_r = getvehfield_any(veh, {'Kphi_R_total','Kphi_r','Kphi_rear'}, ...
          veh.Kphi * (1 - veh.dis_Kphi_ratio));

Cphi_f = getvehfield_any(veh, {'Cphi_F','Cphi_f','Cphi_front'}, ...
          veh.Cphi * veh.dis_Cphi_dot_ratio);
Cphi_r = getvehfield_any(veh, {'Cphi_R','Cphi_r','Cphi_rear'}, ...
          veh.Cphi * (1 - veh.dis_Cphi_dot_ratio));

% Use the explicitly split front/rear values to define the total restoring
% roll moment. This guarantees consistency between the roll ODE and the
% wheel-load transfer calculation.
Kphi_total = Kphi_f + Kphi_r;
Cphi_total = Cphi_f + Cphi_r;
Mphi_sus2unspr_dyn = Cphi_total * phi_roll_dot + Kphi_total * phi_roll;
...
( m_s*h_roll_s*ay_bar ...
- Cphi_total*phi_roll_dot - Kphi_total*phi_roll ) / Ix_sprung;
