...
f_dyn = Function('f_dyn',{x,u,pv},{dx,sf},{'x','u','pv'},{'dx','sf'});
...
% 
% murl_ineq = 0.9 - murl^2;
% murr_ineq = 0.9 - murr^2;
...
pwr_ub = (372/2)*10^3/(Tt_s*omega_s);

% Collect constraints
% h    = [mufl_ineq mufr_ineq murl_ineq murr_ineq BrTh pwr_rl pwr_rr]';
% h_lb = [  0   0   0   0   0-OPT_e pwr_lb pwr_lb]';
% h_ub = [inf inf inf inf   0+OPT_e pwr_ub pwr_ub]';

h    = [BrTh pwr_rl pwr_rr]';
h_lb = [0-OPT_e pwr_lb pwr_lb]';
h_ub = [0+OPT_e pwr_ub pwr_ub]';
...
% Path constraints
h_eq = Function('h_eq', {x, u, pv}, {h}, {'x','u','pv'}, {'h'});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
haha = 0;
n = d*haha;                  % number of elements to move
kappa_col_shifted = [kappa_col(n+1:end), kappa_col(1:n)];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Xi = [V_start; nan; nan; nan; nan; nan; nan; nan; nan; nan; nan];
%-final
Vf = nan;
% Xf = [Vf; nan; nan; nan; nan; nan; nan; nan; nan];
Xf = [Vf; nan; nan; nan; nan; nan; nan; nan; nan; nan; nan];
...
index_1_corners_a = 192*2 - haha; % 2m interval
index_2_corners_a = 227*2 - haha; % 2m interval

index_1_corners_b = 271*2 - haha; % 2m interval
index_2_corners_b = 297*2 - haha; % 2m interval

index_1_corners_c = 555*2 - haha; % 2m interval
index_2_corners_c = 651*2 - haha; % 2m interval

index_1_corners_d = 888*2 - haha;
index_2_corners_d = 936*2 - haha;

index_1_corners_e = 1145*2 - haha;
index_2_corners_e = 1165*2 - haha;

index_1_corners_f = 1251*2 - haha;
index_2_corners_f = 1297*2 - haha;

index_1_corners_g = 1475*2 - haha;
index_2_corners_g = 1531*2 - haha;

index_1_corners_h = 1624*2 - haha;
index_2_corners_h = 1714*2 - haha;

index_1_corners_i = 1778*2 - haha;
index_2_corners_i = 1805*2 - haha;
...
Wbeta = 0.0;        % drift weight  200.0   75.0   90.0
...
k1_corners_c = index_1_corners_c - 1;
k2_corners_c = index_2_corners_c - 1;

k1_corners_d = index_1_corners_d - 1;
k2_corners_d = index_2_corners_d - 1;

k1_corners_e = index_1_corners_e - 1;
k2_corners_e = index_2_corners_e - 1;

k1_corners_f = index_1_corners_f - 1;
k2_corners_f = index_2_corners_f - 1;

k1_corners_g = index_1_corners_g - 1;
k2_corners_g = index_2_corners_g - 1;

k1_corners_h = index_1_corners_h - 1;
k2_corners_h = index_2_corners_h - 1;

k1_corners_i = index_1_corners_i - 1;
k2_corners_i = index_2_corners_i - 1;
...
x0 = [V_0; beta_0; gamma_0; n_0; xi_0; ax_bar_0; ay_bar_0; omega_fl_0; omega_fr_0; omega_rl_0; omega_rr_0]./x_s;
xc0 = reshape(kron(x0(:,1:end-1),ones(1,d)),d*N*nx,1); %constant interpolation between x0
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
u0 = [Tt_0; Tb_0; delta_0]./u_s;
% u0 = [u_pedal_0; delta_0]./u_s;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
w_drift_corners_c = zeros(1,N);
w_drift_corners_d = zeros(1,N);
w_drift_corners_e = zeros(1,N);
w_drift_corners_f = zeros(1,N);
w_drift_corners_g = zeros(1,N);
w_drift_corners_h = zeros(1,N);
w_drift_corners_i = zeros(1,N);
...
for kk = 0:N-1
    ...
    w_drift_corners_c(kk+1) = sig((kk - k1_corners_c)/k_trans) * sig((k2_corners_c - kk)/k_trans);
    w_drift_corners_d(kk+1) = sig((kk - k1_corners_d)/k_trans) * sig((k2_corners_d - kk)/k_trans);
    w_drift_corners_e(kk+1) = sig((kk - k1_corners_e)/k_trans) * sig((k2_corners_e - kk)/k_trans);
    w_drift_corners_f(kk+1) = sig((kk - k1_corners_f)/k_trans) * sig((k2_corners_f - kk)/k_trans);
    w_drift_corners_g(kk+1) = sig((kk - k1_corners_g)/k_trans) * sig((k2_corners_g - kk)/k_trans);
    w_drift_corners_h(kk+1) = sig((kk - k1_corners_h)/k_trans) * sig((k2_corners_h - kk)/k_trans);
    w_drift_corners_i(kk+1) = sig((kk - k1_corners_i)/k_trans) * sig((k2_corners_i - kk)/k_trans);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for k = 0:N-1
    ...
    wk_corners_c = w_drift_corners_c(k+1);
    wk_corners_d = w_drift_corners_d(k+1);
    wk_corners_e = w_drift_corners_e(k+1);
    wk_corners_f = w_drift_corners_f(k+1);
    wk_corners_g = w_drift_corners_g(k+1);
    wk_corners_h = w_drift_corners_h(k+1);
    wk_corners_i = w_drift_corners_i(k+1);
...
% %-decision variables
w = [Xk(:); Uk(:); Xkj(:)]; % Collect all the decision variables [states; inputs; helper states]
lbw = [Xk_min; repmat(u_min(:),N+1,1); Xkj_min]; % Lower bounds
ubw = [Xk_max; repmat(u_max(:),N+1,1); Xkj_max]; % Upper bounds
w0 = [x0(:); u0(:); xc0(:)]; % Collect initial guesses for the decision variables
...
f_dt_opt = Function('f_dt_opt',{w},{dt});
f_aero = Function('f_aero',{x,u,pv}, {y_aero});
f_tyre = Function('f_tyre',{x,u,pv},{y_tyre});
f_slip = Function('f_alpha',{x,u,pv},{y_slip});
f_acc = Function('f_acc',{x,u,pv},{y_acc});
f_torque = Function('f_torque',{x,u,pv},{y_torque});
f_mu = Function('f_mu',{x,u,pv},{y_mu});
f_pwr = Function('f_pwr',{x,u,pv},{y_pwr});
f_xdot = Function('f_xdot',{x,u,pv},{y_xdot});
...
% hold on
% plot(-w_drift_corners_c*beta_ref)
% hold on
% plot(w_drift_corners_d*beta_ref)
% hold on
% plot(w_drift_corners_e*beta_ref)
% hold on
% plot(-w_drift_corners_f*beta_ref)
% hold on
% plot(w_drift_corners_g*beta_ref)
% hold on
% plot(w_drift_corners_h*beta_ref)
% hold on
% plot(w_drift_corners_h*beta_ref)
% legend('track.kappa at knots', '\beta desired at collocation points', '\beta solution', '\delta solution', 'wdrift window scaled', 'wdrift window scaled', 'wdrift window scaled', 'wdrift window scaled', 'wdrift window scaled', 'wdrift window scaled', 'wdrift window scaled', 'wdrift window scaled', 'wdrift window scaled')
xlabel('Index')
...
sol.aero = full(f_aero(sol.x_opt./x_s,sol.u_opt./u_s,kappa_knot))';
sol.tyre = full(f_tyre(sol.x_opt./x_s,sol.u_opt./u_s,kappa_knot))';
sol.slip = full(f_slip(sol.x_opt./x_s,sol.u_opt./u_s,kappa_knot))';
sol.acc = full(f_acc(sol.x_opt./x_s,sol.u_opt./u_s,kappa_knot))';
sol.torque = full(f_torque(sol.x_opt./x_s,sol.u_opt./u_s,kappa_knot))';
sol.mu = full(f_mu(sol.x_opt./x_s,sol.u_opt./u_s,kappa_knot))';
sol.pwr = full(f_pwr(sol.x_opt./x_s,sol.u_opt./u_s,kappa_knot))';
sol.xdot = (full(f_xdot(sol.x_opt./x_s,sol.u_opt./u_s,kappa_knot)).*x_s)';
