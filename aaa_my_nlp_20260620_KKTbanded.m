...
% pw_drive_rear_normalized = Td_state * (omega_rl+omega_rr)/2 / (Td_state_s*omega_s);

pw_drive_rear_ub_normalized = veh.power_motor_max / (Tt_s*omega_s); % [Watts] / [Watts]

% pw_drive_rear_ub_normalized = veh.power_motor_max / (Td_state_s*omega_s); % [Watts] / [Watts]

BrTh = Tt_n*Tb_n;

% BrTh = Td_state_n*Tb_n;
...
% d = 3; %-degree of interpolating poly

% 

% % Colloc points in normalised interval [0,1]
...
% x0 = [ux_0; uy_0; r_0; n_0; xi_0; ax_bar_0; ay_bar_0; omega_fl_0; omega_fr_0; omega_rl_0; omega_rr_0; theta_0; phi_0; thetadot_0; phidot_0]./x_s;
...
[dXkj_col, Qkj_col] = f_dyn_col(Xkj, Uk_col, Zkj, kappa_col_vec);
h_col_path = h_eq_col(Xkj, Uk_col, Zkj, kappa_col_vec);
...
% Xk_min(4:nx:end) = full(-wr_interp(step)+(veh.wt/2+veh.ws/2))/n_s;

% Xk_max(4:nx:end) = full(wl_interp(step)-(veh.wt/2+veh.ws/2))/n_s;
...
% Xkj_min(4:nx:end) = full(-wr_interp(step_col)+(veh.wt/2+veh.ws/2))/n_s;

% Xkj_max(4:nx:end) = full(wl_interp(step_col)-(veh.wt/2+veh.ws/2))/n_s;
...
h_knot_path = h_eq_knot(Xk, Uk, Zk, kappa_knot_vec);
ghk = {h_knot_path(:)};
...
f_sf_knot = f_sf.map(N);
sf_knot = f_sf_knot(Xk(:,1:N), kappa_knot_vec(1:N));
dsdt_knot = 1 ./ sf_knot;
% Same as interval-by-interval {duk(:,k)/sf_k}, stacked by interval.
gduk_mat = duk .* (ones(nu,1) * dsdt_knot);
gduk = {reshape(gduk_mat, nu*N, 1)};
...
g_interval_stage = [gck_stage; ghc_stage; gduk_mat; h_knot_path(:,2:N+1)];
g = [Xk(:,1); h_knot_path(:,1); g_interval_stage(:); Xk(:,N+1)];

lbg_interval_stage = [zeros((d+1)*nx,1); repmat(h_lb,d,1); duk_lb; h_lb];
ubg_interval_stage = [zeros((d+1)*nx,1); repmat(h_ub,d,1); duk_ub; h_ub];
...
% correspondingly enforce the intial total speed
...
opts.ipopt.max_iter = 1500; %max number of iterations.
...
% com_x_opt = track.x + diag(nor(:,1))*n_opt;

% com_y_opt = track.y + diag(nor(:,2))*n_opt;

% 

% [s_opt, ~] = calc_length_xy([com_x_opt, com_y_opt]);

% 

% [phi_opt, kappa_opt] = calc_head_curv_num([com_x_opt, com_y_opt]);

% 

% figure();plot(nor(:,1));hold on;plot(nor(:,2));legend('nor1', 'nor2')

% figure();plot(track.x, track.y, '-o');axis equal

% figure();plot(track.Theta, '-o')

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% 

% sol.track_opt = [com_x_opt com_y_opt s_opt phi_opt kappa_opt];

% 

% figure()

% plot(P_L_interp_here(:,1), P_L_interp_here(:,2))

% hold on

% plot(P_R_interp_here(:,1), P_R_interp_here(:,2))

% hold on

% plot(com_x_opt, com_y_opt)
